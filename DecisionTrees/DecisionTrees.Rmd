...
PatientData <- read.csv('C:\\MOOC\\Machine Learning\\DecisionTrees\\ilpd_data.csv')
...
library(caret)
set.seed(007)
train_index <- createDataPartition(PatientData$disease, p = 0.7, list = F )
train_data <- PatientData[train_index, ] 
test_data <- PatientData[-train_index, ]
...
test_data <- knnImputation(test_data, k=5, scale = T)
...
**NOTE** Clear your environment, you can do that by running the following code chunk
...
HeatData <- read.csv('C:\\MOOC\\Machine Learning\\DecisionTrees\\building_energy.csv')
...
train_index <- createDataPartition(HeatData$heating_load, p= 0.7, list = F)
traindata <- HeatData[train_index, ]
testdata <- HeatData[-train_index, ]
...
library(rpart)
reg_tree <- rpart(heating_load ~., traindata)
...
preds_reg <- predict(reg_tree, testdata)
library(DMwR)
regr.eval(testdata$heating_load, preds_reg)
